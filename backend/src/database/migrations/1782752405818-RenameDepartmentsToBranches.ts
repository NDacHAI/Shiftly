...
export class RenameDepartmentsToBranches1782752405818 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        const hasPositions = await queryRunner.hasTable('positions');
        const hasBranches = await queryRunner.hasTable('branches');

        if (hasPositions) {
            const positions = await queryRunner.getTable('positions');
            const departmentForeignKey = positions?.foreignKeys.find(
                (foreignKey) => foreignKey.name === 'FK_positions_department',
            );
            const departmentIndex = positions?.indices.find(
                (index) => index.name === 'IDX_positions_department_id',
            );

            if (departmentForeignKey) {
                await queryRunner.dropForeignKey('positions', departmentForeignKey);
            }

            if (departmentIndex) {
                await queryRunner.dropIndex('positions', departmentIndex);
            }

            if (
                (await queryRunner.hasColumn('positions', 'department_id')) &&
                !(await queryRunner.hasColumn('positions', 'branch_id'))
            ) {
        ...
        if (hasPositions && (await queryRunner.hasColumn('positions', 'branch_id'))) {
            const positions = await queryRunner.getTable('positions');
            const branchIndex = positions?.indices.find(
                (index) => index.name === 'IDX_positions_branch_id',
            );
            const branchForeignKey = positions?.foreignKeys.find(
                (foreignKey) => foreignKey.name === 'FK_positions_branch',
            );
